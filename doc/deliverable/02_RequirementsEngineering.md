...
| US-06 | Giuseppe - Cittadino (visitatore)         | Come cittadino visitatore voglio visualizzare le segnalazioni sia in forma tabellare che mappa così da poter visualizzare i problemi urbani.|  
...
| FR-9   | Il sistema deve generare una notifica quando una segnalazione cambia di stato                         | Media    | US-03, US-17   | Se una segnalazione cambia di stato deve esser generata automaticamente una notifica per il cittadino segnalante e per i cittadini che hanno scelto di seguire la segnalazione. Deve inoltre essere inviata una email agli utenti che lo hanno richiesto.|
...
| FR-11.3 | Il sistema deve consentire l'ordinamento delle segnalazioni in base a un criterio                      | Bassa    | US-14          | Possibili criteri sono ordine alfabetico, data di inserimento, categoria, stato ecc.|
...
| FR-15   | Il sistema deve gestire il servizio di messaggistica diretta tra cittadino e operatore                | Alta     | US-04, US-16   | Operatori e cittadini autenticati possono scambiarsi messaggi relativi ad una segnalazione tramite piattaforma al fine di richiedere chiarimenti o agiornamenti. Il sistema manterrà traccia dei messaggi scambiati.|
...
| NFR-10 | Disponibilità        | Alta disponibilità per inserimento segnalazioni e consultazioni                                    | L'utilizzo del sito web e delle API pubbliche deve essere garantito per almeno il 99,5% del tempo | Monitoraggio continuo                                               | Alta     | Il servizio deve essere sempre attivo, specialmente per segnalare problemi in situazioni di emergenza urbana  |
...
| NFR-13 | Sicurezza (Security) | Le password degli utenti devono essere crittografate e memorizzate in modo sicuro                   | Utilizzo di algoritmi di hashing | Security Audit e Test di penetrazione| Alta| Fondamentaler per garantire un'adeguata protezione dei dati utente.|
| NFR-14 | Manutenibilità | L'architettura del sistema deve essere modulare| Il sistema deve essere progettato con una netta separazione tra frontend e backend. | Revisione di codice e analisi dell'architettura | Media | Facilita la manutenzione e l'estensione futura del sistema.|
| NFR-15 | Scalabilità | Il sistema deve essere scalabile | Il sistema deve essere in grado di gestire un aumento degli utenti e delle segnalazioni senza degrado delle prestazioni | Monitoraggio delle performance | Media | Garantisce che il sistema possa crescere con l'aumento dell'adozione da parte dei cittadini.|
| NFR-16 | Scalabilità | Il sistema deve permettere di gestire simultaneamente 80000 utenti | | Alta | Il sistema deve essere progettato per supportare un numero elevato di utenti contemporaneamente, considerando che Torino ha più di 800000 abitanti.|
