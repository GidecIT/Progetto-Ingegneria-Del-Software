...
| NFR-03 | Sicurezza  | Protezione delle password|Le password degli utenti devono essere crittografate e memorizzate in modo sicuro tramite l'utilizzo di algoritmi di hashing | Security Audit e Test di penetrazione| Alta| Fondamentaler per garantire un'adeguata protezione dei dati utente.|
| NFR-04 | Sicurezza  | Tutela dell'anonimato pubblico| Nessuna occorrenze di dati identificativi ( nome,cognome,email) nei payload API pubblici| Analisi statica del codice e Penetration Test| Alta | Protezione della privacy del cittadino| 
...
| NFR-07 | Usabilità            | Web app responsive| Layout che si adatta senza scrolling orizzontale su schermi da minimo 320 pixel di larghezza| Ispezione visiva e UI test| Alta     | Essenziale poichè la maggior parte delle segnalazioni avvengono direttamente in strada|
...
| NFR-14 | Qualità | Esportazione dati tabellari in formato CSV| Conformità totale allo standard RFC 4180, nessun errore in Excel/Sheets| Ispezione tramite validatori CSV| Media    | Garantisce trasparenza e usabilità dei dati offline per analisi esterne|