...
| PER-04 | Giuseppe  | Cittadino (visitatore)  | Cittadino anziano di 78 anni che ha difficoltà con l'utilizzo di dispositivi elettronici.   |  Consultare la mappa per vedere se i problemi della sua via sono già stati segnalati.   |   Difficoltà di lettura       | Smartphone (uso saltuario), Wi-Fi domestico. | Font molto grandi; icone intuitive; possibilità di invio segnalazioni in anonimo.               |
...
| PER-06 | Matteo    | Operatore Comunale      | Diendente comunale di 28 anni. Riceve gli incarichi sul tablet e deve recarsi sul luogo per verificare il danno.         | Documentare l'avvenuta riparazione con foto e informazioni.  | Lavora spesso sotto la pioggia o con i guanti; deve operare in zone con poco segnale.            | Tablet rugged (rinforzato), GPS.             | Interfaccia con tasti grandi e utilizzabile con mani bagnate/guanti.                           |
...
|:------|:----------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| US-01 | Giuseppe - Cittadino (visitatore)   | Come cittadino visitatore voglio visualizzare la mappa delle segnalazioni così che io possa informarmi sui problemi del mio quartiere.   
...
| US-07 | Marco - Cittadino (autenticato)   | Come cittadino, voglio avere accesso ad una pagina contente le segnalazioni effettuate ed avere accesso ai dettagli. Voglio inoltre la possibilità di esportare le segnalazioni in CSV per condurre un analisi privata |
...
| US-12 | Giulia - Operatore comunale| Come operatore, voglio poter rifiutare segnalazioni duplicate aggiungendo una motivazione ed inoltre voglio poter comunicare direttamente con il cittadino tramite il servizio di messagistica. |
| US-13 | Sandro - Amministratore           | CConsultare i log di sistema per diagnosticare bug applicativi e monitorare tentativi di intrusione.
...
|:------|:------------------------------------------|:---------|:--------------|:------|
| FR-1 | Il sistema deve consentire la creazione di molteplici account con privilegi di amministratore | alta | US-13 | Il primo account amministratore viene creato durante la creazione del sistema e poi ogni amministratore può creare altri amministratori |
| FR-2 | Il sistema deve consentire ai cittadini di creare l'account | alta | US-01  | I cittadini per registrarsi devono fornire le informazioni identificative di base: username, nome, cognome ed inoltre devono confermare l'indirizzo email tramite un link di verifica |
| FR-2 | Il sistema deve registrare in un file di log ogni errore applicativo e ogni tentativo di accesso fallito  | alta |   | I log devono essere persistenti e non modificabili. Devono tracciare errori applicativi e accessi falliti per scopi di debugging e monitoraggio della sicurezza, garantendo la continuità operativa del sistema. |
| FR-3 | Il sistema deve consentire agli amministratori di creare e gestire gli account per gli operatori comunali | alta | US-13 |  |
| FR-4 | Il sistema deve consentire agli utenti registrati di effettuare il login | alta | US-01, US-02, US-03, US-04 | Il login viene fatto tramite username/email e password VERIFICARE COME COLLEGARE LE USER STORY|
| FR-4.1 | Il sistema deve consentire agli utenti registrati di effettuare un reset della passoword | media | US-01, US-02, US-03, US-04 | Se qualcuno non ricorda la password tramite un link email deve essere possibile resettarla  |
| FR-5 | Il sistema deve consentire agli utenti loggati di effettuare un logout | alta | US-01, US-02, US-03, US-04 |  |
| FR-5 | Il sistema deve permettere la visione e la modifica dei dati personali per gli account registrati | media |  | I dati personali come email, password, username e preferenza di notifica devono essere visualizzabili e deve esserci la possibilità di cambiarli |
...
| FR-5.2 | Il sistema deve permettere ai cittadini di visualizzare l'elenco delle proprie segnalazioni | media |  | Per i cittadini registrati deve essere possibile accedere ad un area dedicata con tutte le proprie segnalazioni  |
| FR-6| Il sistema deve consentire la visualizzazione della mappa sia in versione grafica sia  tabellare| media  |         | La mappa con eventuali pin per le segnalazioni deve essere visibile sia agli utenti registrati sia ai visitatori non registrati |
| FR-7| Il sistema deve consentire la ricerca tramite un campo e l'ordinamento delle segnalazioni | media |         | L'ordinamento e la ricerca deve essere possibile sia agli utenti registrati sia ai visitatori non registrati |
| FR-8| Il sistema deve consentire l'accesso ad una pagina dedicata al dettaglio delle segnalazioni | alta |         | La pagina con il dettaglio delle segnalazioni può avere: titolo, descrizione, categoria, posizione, foto, stato corrente e storico aggiornamento |
| FR-9| Il sistema deve consentire l'esportazione delle segnalazioni in CSV | bassa |         |  |
| FR-10 | Il sistema deve consentire il follow delle segnalazioni|  media |         | I cittadini registrati devono poter attivare il follow di una qualsiasi segnalazione in modo da seguirne lo stato  |
| FR-11 | Il sistema deve consentire ai cittadini di inserire le segnalazioni|  alta |         | Deve essere possibile solo per i cittadini registrati includendo: titolo, posizione scelta tramite mappa, descrizione, categoria e massimo 3 foto, inoltre il cittadino può decidere di contrassegnare una segnalazione come anonima ed in tal caso la l'indentità non è mostrata |
| FR-12 | Il sistema deve gestire gli stati delle segnalazioni| alta  |         | Il sistema deve gestire gli stati delle segnalazioni: pending approval, assigned, in progress, suspended, rejected, resolved|      | |       | |
| FR-12.3 | Il sistema deve garantire una motivazione nel caso di segnalazione respinta| media  |         | Se una segnalazione viene respinta il sistema deve obbligare l'operatore comunale a dare una motivazione |
| FR-13 | Il sistema deve generare una notifica quando una segnalazione cambia di stato | media  |         | Se una segnalazione cambia di stato deve esser generata una notifica in piattaforma per il cittadino segnalante e per i cittadini che hanno scelto di seguire la segnalazione e deve essere inviata una notifica per email per gli utenti che non hanno disabilitato questa opzione |
| FR-14 | Il sistema deve gestire il servizio di messagistica diretta tra cittadino e operatore | media  |         |  |
| FR-15 | Il sistema deve generare le statistiche | media  |         | Le statistiche pubbliche devono essere visibili sia dagli utenti registrati sia ai non registrati. Le statistiche private sono invece visibili ai soli amministratori. |
...
| NFR-11 (Security)| Sicurezza (safety)| Il sistema deve gestire i dati personali garantendo riservatezza e integrità in conformità al GDPR | Gestione dei dati in accordo con le normative vigenti(GDPR) | Security Audit, Penetration Test e verifica dei registri di trattamento dati. | Alta| |
| NFR-12 (Security)| Sicurezza (security)| Le comunicazioni tra i client e il sistema devono utilizzare il protocollo HTTPS | 100% del traffico cifrato via TLS 1.2 o superiore; Certificato SSL/TLS valido | Cifrato via TLS 1.2 o superiore; Certificato SSL/TLS valido.	Analisi del traffico (Wireshark) e scansione degli endpoint (es. SSL Labs) | Alta| |
