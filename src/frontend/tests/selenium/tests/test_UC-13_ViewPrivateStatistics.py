...
class TestViewPrivateStatistics:
    ...
    def test_at_least_three_metric_columns_present(self, page: PageHelper):
        page.login(ADMIN_EMAIL, ADMIN_PASSWORD)
        page.wait_for_url('/admin')
        page.wait.until(EC.presence_of_element_located((By.XPATH, "//*[contains(@id,'admin-metric-item-')]")), message='No admin-metric-item-* element found')
        grid = page.by_id('admin-stats-grid')
        metric_items = grid.find_elements(By.XPATH, "./div[contains(@id,'admin-metric-item-')]")
    ...
    def test_each_metric_column_renders_even_when_empty(self, page: PageHelper):
        ...
        for item in metric_items:
            ...
            assert item.find_elements(By.XPATH, ".//ul[contains(@id,'-list')]"), f"Metric column '{item_id}' has no list container"
