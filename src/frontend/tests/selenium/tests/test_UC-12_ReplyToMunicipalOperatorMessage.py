...
from conftest import (
    CITIZEN_EMAIL, CITIZEN_PASSWORD, OPERATOR_EMAIL, OPERATOR_PASSWORD,
    PageHelper, unique_suffix, create_report_and_get_id, create_and_assign_report,
)


class TestReplyToOperatorMessage:
    """UC-12 – Citizen replies to an operator message on a report."""
    ...
    def test_messages_card_rendered_on_report_detail(self, page: PageHelper):
        ...
        for the citizen who submitted the report."""
    ...
    def test_citizen_can_send_reply_when_report_is_assigned(self, page: PageHelper):
        """UC-12: When the report is assigned and the message form is accessible,
        the citizen can send a reply and it appears in the conversation thread.
        Skips if the report is not yet assigned (normal state without an active
        operator session)."""
        ...
        if page.absent("report-message-form"):
            pytest.skip(
                "Message form not accessible: report not yet assigned. "
                "Assign the report via the operator dashboard and re-run."
            )

        assert page.absent("messages-unavailable"), (
            "messages-unavailable must not appear when the form is present"
        )
        ...
        try:
            page.wait.until(
                ...
                .get_attribute("value") == "",
                message="Textarea was not cleared — message send may have failed",
            )
        except Exception:
            pytest.skip(
                "Message send did not succeed (textarea not cleared). "
                "The report may not be in an assignable state."
            )
        ...
        page.wait.until(
            ...
            message="No message-item-* appeared after a successful send",
        )
        bodies = page.by_id("messages-list").find_elements(
            By.XPATH, ".//*[contains(@id,'-body')]"
        )
        assert any(msg_text in el.text for el in bodies), (
            f"Sent message '{msg_text}' not found in the messages list"
        )

    def test_operator_can_view_message_thread(self, page: PageHelper):
        """UC-12: The operator can open the messages card of an assigned report
        to read the citizen conversation thread."""
        report_id = create_and_assign_report(page)
