"""
Navigation and access-control smoke tests.

These cover the shell navigation links, the 404 page, and the protected-route
guard across all roles – they complement (rather than duplicate) the UC tests.
"""
import pytest
...
class TestNavigation:
    ...
    def test_guest_sees_login_and_register_nav(self, page: PageHelper):
        """Unauthenticated users see Login and Register in the nav bar."""
        page.go("/")
        page.by_id("nav-login")
        page.by_id("nav-register")

    def test_citizen_nav_links(self, page: PageHelper):
        """Citizen sees New Report, Dashboard but NOT Operator or Admin links."""
    ...
    def test_operator_nav_links(self, page: PageHelper):
        """Operator sees Operator and Dashboard links but NOT Admin or New Report."""
    ...
    def test_admin_nav_links(self, page: PageHelper):
        """Admin sees Operator and Admin links in the nav bar."""
    ...
    def test_logout_button_visible_when_logged_in(self, page: PageHelper):
        """The Logout button is present for any authenticated user."""
        page.login(CITIZEN_EMAIL, CITIZEN_PASSWORD)
        page.by_id("logout-button")

    def test_swagger_link_always_present(self, page: PageHelper):
        """The Swagger API docs link is always shown in the navigation."""
        page.go("/")
